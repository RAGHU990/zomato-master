...
import { useParams, Link } from 'react-router-dom';import classname from "classnames";
    ...
    return <> 
             <div className="hidden lg:flex gap-14 container px-20 mx-auto">
               ...
               <div className={classNames(
                   "flex items-center gap-3 pb-2 transition duration-700", 
               {"border-b-2 border-zomato-400"
                : type=== items.id,})}>

                     <div className={classNames("w-16 h-16 bg-gray-100  p-4 rounded-full",{[`bg-${items.activeColor}-100`]
                      :type ===items.id })
                     
                      }>
                         <img
                          src={type === items.id 
                              ? items.imageActive : items.imageDefault}
                         alt="delivery" 
